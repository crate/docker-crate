...
RUN yum install -y yum-utils https://centos7.iuscommunity.org/ius-release.rpm \
    ...
    && yum clean all \
    && rm -rf /var/cache/yum \
    && curl -fSL -O https://cdn.crate.io/downloads/releases/crate-4.0.8.tar.gz \
    && curl -fSL -O https://cdn.crate.io/downloads/releases/crate-4.0.8.tar.gz.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 90C23FC6585BC0717F8FBFC37FAAE51A06F6EAEB \
    && gpg --batch --verify crate-4.0.8.tar.gz.asc crate-4.0.8.tar.gz \
    && rm -rf "$GNUPGHOME" crate-4.0.8.tar.gz.asc \
    && tar -xf crate-4.0.8.tar.gz -C /crate --strip-components=1 \
    && rm crate-4.0.8.tar.gz \
