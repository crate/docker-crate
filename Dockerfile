...
RUN groupadd crate \
    ...
    && export CRATE_URL=https://cdn.crate.io/downloads/releases/cratedb/${PLATFORM}/crate-5.4.4.tar.gz \
    && curl -fSL -O ${CRATE_URL} \
    && curl -fSL -O ${CRATE_URL}.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 90C23FC6585BC0717F8FBFC37FAAE51A06F6EAEB \
    && gpg --batch --verify crate-5.4.4.tar.gz.asc crate-5.4.4.tar.gz \
    && rm -rf "$GNUPGHOME" crate-5.4.4.tar.gz.asc \
    && tar -xf crate-5.4.4.tar.gz -C /crate --strip-components=1 \
    && rm crate-5.4.4.tar.gz
...
LABEL maintainer="Crate.io <office@crate.io>" \
    org.opencontainers.image.created="2023-10-13T08:53:58.719371" \
