...
RUN set -ex \
    ...
    && wget -nv "$CDN/sigar/libsigar-amd64-linux.so.1.0" -P /usr/local/lib \
    && ln /usr/local/lib/libsigar-amd64-linux.so.1.0 /usr/local/lib/libsigar-amd64-linux.so.1 \
    && runDeps="$(\
...
RUN ln -sf /usr/bin/python3 /usr/bin/python \
    && ln -sf /usr/local/lib/libsigar-amd64-linux.so.1 /crate/lib/sigar/libsigar-amd64-linux.so
