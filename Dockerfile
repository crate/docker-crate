...
RUN apk add --no-cache --virtual .crate-rundeps \
    ...
    && apk add --no-cache --virtual .build-deps \
        gnupg \
        tar \
    && curl -fSL -O https://cdn.crate.io/downloads/releases/crate-3.1.4.tar.gz \
    && curl -fSL -O https://cdn.crate.io/downloads/releases/crate-3.1.4.tar.gz.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 90C23FC6585BC0717F8FBFC37FAAE51A06F6EAEB \
    && gpg --batch --verify crate-3.1.4.tar.gz.asc crate-3.1.4.tar.gz \
    && rm -rf "$GNUPGHOME" crate-3.1.4.tar.gz.asc \
    && tar -xf crate-3.1.4.tar.gz -C /crate --strip-components=1 \
    && rm crate-3.1.4.tar.gz \
...
LABEL maintainer="Crate.io <office@crate.io>" \
    ...
    org.label-schema.build-date="2018-12-19T12:39:46.068316553+00:00" \
