...
RUN groupadd crate \
    ...
    && export CRATE_URL=https://cdn.crate.io/downloads/releases/cratedb/${PLATFORM}/crate-5.2.10.tar.gz \
    && curl -fSL -O ${CRATE_URL} \
    && curl -fSL -O ${CRATE_URL}.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 90C23FC6585BC0717F8FBFC37FAAE51A06F6EAEB \
    && gpg --batch --verify crate-5.2.10.tar.gz.asc crate-5.2.10.tar.gz \
    && rm -rf "$GNUPGHOME" crate-5.2.10.tar.gz.asc \
    && tar -xf crate-5.2.10.tar.gz -C /crate --strip-components=1 \
    && rm crate-5.2.10.tar.gz

# Install crash
RUN curl -fSL -O https://cdn.crate.io/downloads/releases/crash_standalone_0.30.0 \
    && curl -fSL -O https://cdn.crate.io/downloads/releases/crash_standalone_0.30.0.asc \
    && export GNUPGHOME="$(mktemp -d)" \
    && gpg --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys 90C23FC6585BC0717F8FBFC37FAAE51A06F6EAEB \
    && gpg --batch --verify crash_standalone_0.30.0.asc crash_standalone_0.30.0 \
    && rm -rf "$GNUPGHOME" crash_standalone_0.30.0.asc \
    && mv crash_standalone_0.30.0 /usr/local/bin/crash \
...
LABEL maintainer="Crate.io <office@crate.io>" \
    org.opencontainers.image.created="2023-10-12T15:07:59.910862" \
    ...
    org.opencontainers.image.version="5.2.10"
